...
public void Event_PlayerChangedTeam(Event event,
...
public void Event_PlayerDied(Event event,
	const char[] name, bool dontBroadcast) {
	initiateUpdate();
}
		...
		for (new client = 1; client <= MaxClients; client++) {
			if (IsClientInGame(client) && !IsFakeClient(client) && IsClientAuthorized(client)) {
				if (GetTeamScore(3) >= GetTeamScore(2)) {
					...
					SetHudTextParamsEx(-1.0, 0.01, 999.0, {0, 0, 255, 150}, {0, 0, 255, 150}, 0, 0.0, 0.0, 0.0);
					ShowHudText(client, 254, "№1: Team Combine (score %d)", GetTeamScore(2));
					SetHudTextParamsEx(-1.0, 0.05, 999.0, {255, 0, 0, 150}, {255, 0, 0, 150}, 0, 0.0, 0.0, 0.0);
					ShowHudText(client, 255, "№2: Team Rebels  (score %d)", GetTeamScore(3));
				}
			}
		}
	}
